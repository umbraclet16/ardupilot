 ...
 * shows up in the photo. Also we need a FLAG in AUTO mode to make sure we can only change to
 ...
 *
 * TODO: we may not want to modify MissionPlanner, so maybe we should just replace the 
 * enumeration of an existent flight mode(e.g. altHold) rather than add a new one.
 *
